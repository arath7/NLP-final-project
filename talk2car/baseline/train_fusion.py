...
def main():
    ...
    fusion_module = MultimodalFusion(visual_dim=512, text_dim=512, hidden_dim=256)
    img_encoder.cuda()
    text_encoder.cuda()
    fusion_module.cuda()
    ...
    params = list(text_encoder.parameters()) + list(img_encoder.parameters()) + list(fusion_module.parameters())
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Train
        train(train_dataloader, img_encoder, text_encoder, fusion_module, optimizer, criterion, epoch, args)
        
        # Update lr rate
        scheduler.step()
        
        # Evaluate
        ap50, iou, top1 = evaluate(val_dataloader, img_encoder, text_encoder, fusion_module, args)
...
def train(train_dataloader, img_encoder, text_encoder, fusion_module, optimizer, criterion,
            epoch, args):
...
@torch.no_grad()
def evaluate(val_dataloader, img_encoder, text_encoder, fusion_module, args):
    m_top1 = AverageMeter('Acc@1', ':6.2f')
    m_iou = AverageMeter('IoU', ':6.2f')
    m_ap50 = AverageMeter('AP50', ':6.2f')
    progress = ProgressMeter(
